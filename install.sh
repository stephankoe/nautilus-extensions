...
cat "${SCRIPT_PATH}/utils"/*.py \
    <(echo "_T = \\") \
    <(python3 "${SCRIPT_PATH}/tools/convert_yaml_to_json.py" "${SCRIPT_PATH}/${ext_name}/strings.yaml") \
    <(sed "/from utils/d;/import utils/d;/from ${ext_name}/d;/import ${ext_name}/d" "${SCRIPT_PATH}/${ext_name}"/*.py) \
